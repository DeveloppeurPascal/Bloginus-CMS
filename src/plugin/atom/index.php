...
<feed xmlns="http://www.w3.org/2005/Atom">
	...
	<author><name><?php print(htmlentities(config_getvar("auteur"),ENT_COMPAT,"UTF-8")); ?></name></author>
	...
	if (is_array($articles))
	{
		...
		while(list($key,$value)=each($articles))
		{
			if ($value["published"])
			{
				...
				if (is_array($article))
				{
?><entry><title><?php print(htmlentities($article["label"],ENT_COMPAT,"UTF-8")); ?></title>
			...
			<summary><?php print(substr(trim(strip_tags($article["text"])),0,200)." (...)"); ?></summary>
        </entry>
<?php
				}
			}
